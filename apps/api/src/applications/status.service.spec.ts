...
function employer(company = companyId): TransitionActor {
  return { type: 'EMPLOYER', userId: EMPLOYER_USER_ID, role: UserRole.EMPLOYER, companyId: company };
}
...
describe('StatusService — employer matrix (exhaustive)', () => {
  for (const from of ALL) {
    for (const to of ALL) {
      ...
      if (legal) {
        gatedIt(`LEGAL ${from} → ${to}: writes status + timeline + audit + notification`, async () => {
          const id = await makeApp(from);
          const res = await service.transition(id, to, employer());

          expect(res.status).toBe(to);

          const timeline = await prismaClient.applicationTimelineEntry.findMany({ where: { applicationId: id } });
          ...
          const audits = await prismaClient.auditLog.findMany({ where: { action: 'application.status.changed' } });
          expect(audits).toHaveLength(1);

          // In-app row always; WhatsApp only for a first SELECTED entry; email for all three.
          const inApp = await prismaClient.notification.count({ where: { userId: CANDIDATE_USER_ID } });
          expect(inApp).toBe(1);
        ...
        gatedIt(`ILLEGAL ${from} → ${to}: 422 + three absences (state, timeline, notification)`, async () => {
          const id = await makeApp(from);
          await expect(service.transition(id, to, employer())).rejects.toMatchObject({
            response: { code: 'ILLEGAL_TRANSITION', meta: { from, to, allowed: EMPLOYER_LEGAL[from] } },
          });

          const app = await prismaClient.application.findUnique({ where: { id } });
          expect(app!.status).toBe(from); // no state change
          expect(await prismaClient.applicationTimelineEntry.count({ where: { applicationId: id } })).toBe(0);
          expect(await prismaClient.notification.count({ where: { userId: CANDIDATE_USER_ID } })).toBe(0);
...
describe('StatusService — admin override', () => {
  gatedIt('every corrective move succeeds WITH a reason (timeline flagged + reason + ADMIN_OVERRIDE audit)', async () => {
    ...
    await service.transition(id, ApplicationStatus.REJECTED, admin(), { overrideReason: 'candidate withdrew offline' });

    const app = await prismaClient.application.findUnique({ where: { id } });
    expect(app!.status).toBe(ApplicationStatus.REJECTED);

    const timeline = await prismaClient.applicationTimelineEntry.findFirst({ where: { applicationId: id } });
    ...
    const audits = await prismaClient.auditLog.findMany({ where: { action: 'application.admin_override' } });
    expect(audits).toHaveLength(1);
  ...
  gatedIt('candidate notification carries NO overrideReason, but the timeline row stores it', async () => {
    const id = await makeApp(ApplicationStatus.PENDING);
    await service.transition(id, ApplicationStatus.SHORTLISTED, admin(), { overrideReason: 'manual correction' });

    const notif = await prismaClient.notification.findFirst({ where: { userId: CANDIDATE_USER_ID } });
...
describe('StatusService — one-WhatsApp guard', () => {
  ...
  gatedIt('re-entry (admin SELECTED→REJECTED→SELECTED) sends email+in-app, ZERO new WhatsApp, guard unchanged', async () => {
    const id = await makeApp(ApplicationStatus.PENDING);
    await service.transition(id, ApplicationStatus.SELECTED, employer()); // first entry: WhatsApp fires
    const firstGuard = (await prismaClient.application.findUnique({ where: { id } }))!.selectedNotifiedAt;
    expect(firstGuard).not.toBeNull();

    queueAdd.mockClear();
    await service.transition(id, ApplicationStatus.REJECTED, admin(), { overrideReason: 'reopen flow' });
    await service.transition(id, ApplicationStatus.SELECTED, admin(), { overrideReason: 'reinstated' });
  ...
  gatedIt('concurrent PENDING→SELECTED race: one 200 + one 422, one timeline row, one WhatsApp, guard set once', async () => {
    const id = await makeApp(ApplicationStatus.PENDING);
    ...
    expect(await prismaClient.applicationTimelineEntry.count({ where: { applicationId: id } })).toBe(1);
    expect(enqueuedChannels().filter((c) => c === 'whatsapp')).toHaveLength(1);
    const app = await prismaClient.application.findUnique({ where: { id } });
    expect(app!.selectedNotifiedAt).not.toBeNull();
  });
...
describe('StatusService — scoping / archived / rollback', () => {
  ...
  gatedIt('archived application → 422 for both actors', async () => {
    ...
    await expect(service.transition(id, ApplicationStatus.SHORTLISTED, employer())).rejects.toBeInstanceOf(
  ...
  gatedIt('rollback: audit throws inside the tx → NOTHING persists, NO notification enqueued', async () => {
    const id = await makeApp(ApplicationStatus.PENDING);
    ...
    await expect(brittle.transition(id, ApplicationStatus.SELECTED, employer())).rejects.toThrow('audit boom');
