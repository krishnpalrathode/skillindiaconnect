...
import {
  ...
  EmployerViewSource,
} from './mappers/candidate-employer-view.mapper';
import {
  toBrowseCard,
  BrowseCardSource,
  CandidateBrowseCardDto,
} from './mappers/candidate-browse-card.mapper';
...
export class CandidateViewService {
  ...
  async browse(userId: string, dto: BrowseQueryDto): Promise<Paginated<CandidateBrowseCardDto>> {
    await this.getApprovedCompany(userId);

    const { page, pageSize } = resolvePaging(dto.page, dto.pageSize, 50);

    const { data: sources, total } = await this.candidateReadService.browseVisibleCandidates({
  ...
  async viewCandidate(userId: string, candidateId: string): Promise<CandidateEmployerViewDto> {
    ...
    const photoUrl = candidate.photoKey ? await this.storage.presignGet(candidate.photoKey) : null;
