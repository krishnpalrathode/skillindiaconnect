...
beforeAll(async () => {
  try {
    ...
    const employer = new EmployerService(prismaSvc, null as never, { notify: jest.fn() } as never); // StorageService unused here
...
describe('pre-expiry reminders (T-7 / T-1, ledger-deduped)', () => {
  gatedIt(
    'T-7: ACTIVE sub expiring in 6 days → ONE SUBSCRIPTION_EXPIRING with {expiresAt, window:7}; re-run no-ops',
    async () => {
...
describe('ACTIVE past expiresAt → GRACE', () => {
  gatedIt(
    'sets GRACE + graceEndsAt = expiresAt + 7d, notifies with grace framing, audits; re-run no-ops',
    async () => {
      const { companyId, userId } = await mkCompany();
...
describe('GRACE past graceEndsAt → EXPIRED + pause-all-but-most-recent', () => {
  gatedIt(
    '3 active jobs → the latest-PUBLISHED stays ACTIVE (not latest-created), 2 pause with audit rows',
    async () => {
      const { companyId, userId } = await mkCompany();
      const subId = await mkSub(companyId, {
      ...
      expect(
        await prisma.auditLog.count({
          where: { action: AUDIT_ACTIONS.JOB_PAUSED, targetId: paused1 },
        }),
      ).toBe(1);
      expect((await prisma.job.findUniqueOrThrow({ where: { id: survivor } })).status).toBe(
        JobStatus.ACTIVE,
      );
    },
  );
  ...
  gatedIt(
    'renewal in GRACE → back to ACTIVE untouched by the sweep; expiry-paused jobs STAY paused (manual resume only); reminder ladder restarts on the new expiresAt',
    async () => {
      const { companyId, userId } = await mkCompany();
      const subId = await mkSub(companyId, {
        status: SubscriptionStatus.GRACE,
...
describe('sweep bookkeeping', () => {
  ...
  gatedIt(
    'email jobs go through the queue boundary (worker-and-external-sends) — never sent inline',
    async () => {
      queueAdd.mockClear();
      const { companyId } = await mkCompany();
