...
import { WATERMARK_TEXT } from './templates/shared';
...
// ── BRAND WATERMARK, PER TEMPLATE ───────────────────────────────────────────
/*
  Asserted on the PDF BYTES, not the HTML, for the same reason the privacy gate
  is: a watermark that exists in the markup but is dropped by the print path
  (clipped, positioned off-page, or flattened away) protects nobody. The question
  is whether it is IN THE FILE that reaches the employer.

  Two traps make the OBVIOUS assertion here worthless, and both were live —
  green and measuring nothing — before this counted occurrences:

  1. LETTER-SPACING. The mark is set with `letter-spacing`, and PDF text
     extraction turns tracked-out type into "S k i l l   I n d i a", so a plain
     `toContain('Skill India Connect')` never matches the watermark at all.
     Whitespace is squashed out before comparing.

  2. THE FOOTER. Every template already ends with "Generated by Skill India
     Connect", which CONTAINS the mark's text. So a presence check passes on a
     document with no watermark whatsoever — it was green while measuring the
     provenance line. Only a COUNT can tell the two apart.

  One page tiles roughly 40 marks, so a threshold of 10 sits far above what the
  footer alone can produce and far below what a correct render yields.
*/
const MIN_WATERMARK_MARKS = 10;

function countWatermarks(text: string): number {
  // Case-insensitive for the SAME reason expectNameRendered above is: the mark
  // is set in `text-transform: uppercase`, so the bytes read SKILLINDIACONNECT.
  // Matching case-sensitively counted only the footer and reported 1 — a green
  // test measuring the provenance line while the watermark went unchecked.
  const squashed = text.replace(/\s+/g, '').toLowerCase();
  const needle = WATERMARK_TEXT.replace(/\s+/g, '').toLowerCase();
  return squashed.split(needle).length - 1;
}

describe.each(REGISTERED_TEMPLATES)('brand watermark — IN THE PDF BYTES [%s]', (template) => {
  it('tiles the Skill India Connect mark across the page', async () => {
    const text = await renderToText({ ...allOn, template });
    expect(countWatermarks(text)).toBeGreaterThanOrEqual(MIN_WATERMARK_MARKS);
  });

  it('marks the page even when every optional field is hidden', async () => {
    // The privacy toggles must not be able to strip our own attribution — the
    // sparsest possible document is still ours.
    const text = await renderToText({ ...allOff, template });
    expect(countWatermarks(text)).toBeGreaterThanOrEqual(MIN_WATERMARK_MARKS);
  });
});
    ...
    it('the candidate-written summary reaches the PDF BYTES, above the work history', async () => {
      ...
      */
      const text = await extractText(
        await pool.render(
          selectTemplate(template)(toResumeView({ ...source, summary: SUMMARY }, on, null)),
        ),
      );
      /*
