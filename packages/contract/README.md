...
| 0.9.0   | S7-0   | **Resume builder (CR-001) frozen.** The S1-0 resume skeleton specified in full: `ResumeSettings` becomes the driver of the **RESUME-VIEW — the product's THIRD viewer context** (`ResumeView`, per-toggle omission documented; passport number MAY appear here by the candidate's own choice though the employer context never carries it). Defaults corrected at freeze: `showFatherName` default **true** (matches the S1 data design; the draft said false), religion + passport-number stay **false**. `language` enum is **`[en]` only** (hi/ar deferred — values will be ADDED when translated templates ship). Generation is **async**: `POST /generate` → 202 `{generationId, status: PENDING}`; NEW poll target `GET /candidates/me/resume/status` (`ResumeGeneration`: PENDING/READY/FAILED, READY carries a short-expiry signed url); settings apply AT GENERATION — a later change requires a regenerate. `GET /candidates/me/resume` now also returns `current` (the latest generation) — the optional `/current` endpoint folded into the existing read. Delivery: `send-whatsapp` → own verified number only, 422 `RESUME_NOT_READY`, **degrades to `{delivered: EMAIL_FALLBACK}` (202) when not WhatsApp-capable** (replaces the draft's 409 `WHATSAPP_NOT_CAPABLE`), 429 `RESUME_SEND_LIMIT_EXCEEDED` at 5/day; `send-email` → own address only, READY-required, **no dedicated cap** (global authed limit only — the draft's 5/day moved to the WhatsApp send where the cost lives). `NotificationType` gained `RESUME_READY`. **Locked OUT of CR-001:** the public slug page (the dormant `publicSlug` column stays dormant — NO public endpoint), video portfolio endpoints, hi/ar rendering. **Invoice-PDF debt note:** S7-B1 populates the previously-null invoice `pdfKey` with the same render infra — the existing `GET /billing/invoices` `pdfUrl` field (S5 contract, null-safe in S5-F2) simply starts resolving; **no new endpoint and no FE change needed**. |
| 0.8.1   | S6b-F2 | Admin read surfaces the Screen 26 detail pages needed: `GET /admin/jobs/{id}` (`AdminJobDetail` — full job for ANY status + `companyStatus` for the pre-emptive suspended warning), `GET /admin/applications/{id}` (`AdminApplicationDetail` — the admin row + FULL timeline with per-entry `overrideReason`, its only serialization). Documented what S4-B3 already returned on the admin list (`AdminApplicationRow`: `candidateName`/`jobTitle`; `search` param). |
